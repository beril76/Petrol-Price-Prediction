...
dataread<-read.csv("D:\\Beril\\Desktop\\Python_Project\\petrol5.csv", header = T)

#Loading the dataset
data <- read.csv("D:\\Beril\\Desktop\\Python_Project\\Petrol Price Original.csv", header = T)
datapp <- read.csv("D:\\Beril\\Desktop\\Python_Project\\predicted11.csv", header = T)
...
server <- function(input, output){
  ...
  output$plot1a<-renderPlotly({
    ...
    fit_arima1 <- auto.arima(data[[input$select_dev1a]], lambda = "auto")
  ...
  output$plot4<-renderPlotly({
    ...
    fit_arima4 <- auto.arima(data[[input$select_dev4]], lambda = "auto")
